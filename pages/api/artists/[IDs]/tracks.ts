...
import {
  ...
  SpotifyPluralResponse,
  TrackWithFeature,
} from "../../../../lib/types/spotifyapi";

const tracks: NextApiHandler<Array<Array<TrackWithFeature>>> = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  ...
  const artistAlbums = await Promise.all(
    IDs.map(async (ID) => {
      const res = await api.fetcher.get<
        SpotifyPluralResponse<SpotifyAlbumAPIResponse>
      >(`/artists/${ID}/albums`, {
        params: {
  ...
  const artistTracks = await Promise.all(
    artistAlbums.map(async (albums) => {
      const albumDetails = await api.fetcher.get<{
        albums: Array<SpotifyAlbumAPIResponse>;
  ...
  const splitArtistTrackFeatures = await Promise.all(
    splitArtistTracks.map(async (tracks) => {
      const trackFeatures = await Promise.all(
        tracks.map(async (splitTracks) => {
          const res = await api.fetcher.get<{
            audio_features: Array<AudioFeature>;
          }>(
    ...
    return splitTracks
      .map((_, j) => {
        ...
        return tracks.map((_, k) => {
          const trackWithFeature = {
            ...tracks[k],
            ...features.audio_features[k],
          };
